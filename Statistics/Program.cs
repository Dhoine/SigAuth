...
namespace Statistics
{
    ...
    {
        public static Dictionary<int, List<SignatureSampleDeserialized>> Original =
            new Dictionary<int, List<SignatureSampleDeserialized>>();

        public static Dictionary<int, List<SignatureSampleDeserialized>> Forged =
            new Dictionary<int, List<SignatureSampleDeserialized>>();
        ...
        private static void ParseSignatureDb()
        {
            foreach (var filePath in Directory.GetFiles("Signatures"))
            {
                ...
                var num = int.Parse(parameters[1]);
                ...
                foreach (var line in linesList)
                {
                    ...
                    if (splitted[3] == "1")
                    {
                        buffer.Add(new RawPoint
                        {
                            X = double.Parse(splitted[0]), Y = double.Parse(splitted[1]),
                            TimeStamp = long.Parse(splitted[2])
                        });
                    }
                    else
                    {
        ...
        private static void Check()
        {
            ...
            var epw = new Epw();
            sparse.CompareFeatureList = new List<string>
            {
                GlobalConstants.Sin, GlobalConstants.Cos, GlobalConstants.QDir, GlobalConstants.Speed
            };
        ...
        private static void CheckSignature(int key, ISignatureVerification verification,
            List<SignatureSampleDeserialized> originalSignatures, int countOriginal,
            int countForged, TextWriter writer)
        {
            var counter = 0;
            writer.WriteLine("Genuine:\n");
            foreach (var sample in Original[key])
            {
                var res = verification.CheckSignature(originalSignatures, sample.Sample);
                if (res) counter++;
