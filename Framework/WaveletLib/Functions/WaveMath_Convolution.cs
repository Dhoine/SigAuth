...
namespace WaveletStudio.Functions
{
    public static partial class WaveMath
    {
        ...
        /// <returns></returns>
        public static double[] Convolve(ConvolutionModeEnum convolutionMode, double[] input, double[] filter,
            bool returnOnlyValid = true, int margin = 0, ManagedFFTModeEnum mode = ManagedFFTModeEnum.UseLookupTable)
        {
            return convolutionMode == ConvolutionModeEnum.Normal
                ? ConvolveNormal(input, filter, returnOnlyValid, margin)
                : ConvolveManagedFFT(input, filter, returnOnlyValid, margin, mode);
        }
        ...
        /// <param name="margin">Margin to be used if returnOnlyValid is set to true</param>
        /// <returns></returns>
        public static double[] ConvolveNormal(double[] input, double[] filter, bool returnOnlyValid = true,
            int margin = 0)
        {
        ...
        /// <returns></returns>
        public static double[] ConvolveManagedFFT(double[] input, double[] filter, bool returnOnlyValid = true,
            int margin = 0, ManagedFFTModeEnum mode = ManagedFFTModeEnum.UseLookupTable)
        {
            ...
            var size = realSize > 0 && (realSize & (realSize - 1)) == 0
                ? realSize
                : SignalExtension.NextPowerOf2(realSize);
            ...
            for (var i = 0; i < input.Length; i++) inputFFT[i * 2] = input[i];
            for (var i = 0; i < filter.Length; i++) filterFFT[i * 2] = filter[i];
            ...
            ifft2[0] = ifft[0];
            for (var i = 0; i < ifft2.Length - 2; i = i + 1) ifft2[i + 1] = ifft[ifft.Length - 2 - i * 2];
            int start;
            if (returnOnlyValid)
            {
