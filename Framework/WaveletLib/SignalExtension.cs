...
namespace WaveletStudio
{
    ...
    public static class SignalExtension
    {
        ...
        {
            if (input.Length == 0) return MemoryPool.Pool.New<double>(beforeSize + afterSize, true);
            ...
            {
                if (extensionMode == ExtensionMode.SymmetricHalfPoint ||
                    extensionMode == ExtensionMode.AntisymmetricHalfPoint)
                {
                    if (beforeSize > 0)
                    {
                ...
                else if (extensionMode == ExtensionMode.SymmetricWholePoint)
                {
                    ...
                    Array.Copy(input, input.Length - afterSize - 1 < 0 ? 0 : input.Length - afterSize - 1,
                        afterExtension, 0, afterSize);
                    Array.Reverse(afterExtension);
                }
                ...
                else if (extensionMode == ExtensionMode.SmoothPadding0)
                {
                    ...
                    for (var i = 0; i < afterExtension.Length; i++) afterExtension[i] = input[input.Length - 1];
                }
        ...
        {
            if (input.Length <= 2 || size > input.Length) return input;
        ...
        private static double[] GetAntisymmetricWholePointBefore(IList<double> points, int beforeSize)
        {
            ...
            if (points.Count == 1)
            {
                for (var i = 0; i < beforeSize; i++) beforeExtension[i] = points[0] * -1;
        ...
        private static double[] GetAntisymmetricWholePointAfter(IList<double> points, int afterSize)
        {
            ...
            if (points.Count == 1)
            {
                for (var i = 0; i < afterSize; i++) afterExtension[i] = points[0] * -1;
