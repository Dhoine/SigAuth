...
namespace WaveletStudio
{
    ...
    ///     <para>
    ///         Arrays created in ILNumerics.Net will try to reclaim its memory from this pool. If attempt fails, the memory
    ///         is gathered from the managed heap normally.
    ///     </para>
    ///     <para>
    ///         Disposed array objects register their underlying System.Array in the pool for later reusing. The process is
    ///         triggered by the garbage collector or by the user calling the Dispose function available for all array objects.
    ///     </para>
    /// </remarks>
    ...
    public class MemoryPool
    {
        ...
        /// <remarks>
        ///     <para>
        ///         In order to be stored in the pool, the array must meet the minimum array length and must fit into the global
        ///         pool size.
        ///         Null objects or empty arrays or array not suitable for the pool will be silently ignored.
        ///     </para>
        ///     <para>
        ///         If the new array is too large to fit into the remaining pool space, the oldest objects in the pool will be
        ///         released until the object can get registered.
        ///     </para>
        /// </remarks>
        public void RegisterObject<T>(T[] arr)
        {
            ...
            long newSize = arr.Length * Marshal.SizeOf(arr[0]);
            if (newSize >= _maxBytes) return;
            lock (this)
            {
                // clean up to free space for new array 
                while (_curBytes + newSize > _maxBytes) RemoveOldestObject();
        ...
        private void RemoveOldestObject()
        {
            ...
            if (RemoveFromPool(oldest)) _curBytes -= oldest.Length * Marshal.SizeOf(oldest.GetValue(0));
        }
        ...
        ///     <para>
        ///         There is no way of determining, if the array was recalimed from pool or newly created! If you must be sure,
        ///         the element values are set to default(T), call the overloaded version <see cref="New{T}(int,bool,out bool)" />
        ///         instead!
        ///     </para>
        ///     <para>
        ///         If a new array could not get created due to an OutOfMemoryException, a garbage collection is triggered and
        ///         the array is again requested from the pool. If this failes again, another attempt to create the array is done.
        ///         Exceptions may thrown from this last attempt are not catched and transported back to the callee.
        ///     </para>
        /// </remarks>
        public T[] New<T>(int length)
        {
            ...
            catch (OutOfMemoryException)
            {
                ...
                if (length >= _minArrLength && (ret = GetFromPool<T>(length)) != null) return ret;
            }
        ...
        ///     <para>If a suitable System.Array was found in the pool, this object is returned. Otherwise a new array is created.</para>
        ///     <para>
        ///         If the <paramref name="clear">clear </paramref> parameter was set to false, the iscleared parameter can be
        ///         used to determine, if the object was returnd from the pool and may need extra clearing.
        ///     </para>
        ///     <para>
        ///         If a new array could not get created due to an OutOfMemoryException, a garbage collection is triggered and
        ///         the array is again requested from the pool. If this failes again, another attempt to create the array is done.
        ///         Exceptions eventually thrown from this last attempt are not catched and transported back to the callee.
        ///     </para>
        /// </remarks>
        ...
        ///     <para>If a suitable System.Array was found in the pool, this object is returned. Otherwise a new array is created.</para>
        ///     <para>
        ///         If the <paramref name="clear">clear </paramref> parameter was set to false, the
        ///         <paramref name="iscleared">iscleared</paramref> parameter can be used to determine, if the object was returnd
        ///         from the pool and may need extra clearing.
        ///     </para>
        ///     <para>
        ///         If a new array could not get created due to an OutOfMemoryException, a garbage collection is triggered and
        ///         the array is again requested from the pool. If this failes again, another attempt to create the array is done.
        ...
        public T[] New<T>(int length, bool clear, out bool iscleared)
        {
            ...
            if (length >= _minArrLength && (ret = GetFromPool<T>(length)) != null)
            {
                if (clear)
                {
                    for (var i = 0; i < length; i++) ret[i] = default;
                    iscleared = true;
                }
                else
                {
            ...
            lock (this)
            {
                ...
                if (_pool.TryGetValue(length, out inPool))
                    foreach (var arr in inPool)
                    {
                        ...
                        inPool.Remove(arr);
                        _livetimes.Remove(arr);
                        var newSize = arr.Length * Marshal.SizeOf(arr.GetValue(0));
                        _curBytes -= newSize;
                        if (inPool.Count == 0)
                            _pool.Remove(length);
        ...
        /// </summary>
        /// <param name="shortVersion">
        ///     true: abbreviate infos to: current MB in Pool, reclaimed MB for livetime, reclaimed objects
        ///     for livetime. False: give full info (see below)
        /// </param>
        ...
        public string Info(bool shortVersion, bool reset)
        {
            if (shortVersion)
            {
                var s = string.Format("CurMB: {0}| CurObj: {1}| ReclMB: {2}| ReclObj: {3}", _curBytes / 1024 / 1024,
                    CountObj(), ReclaimedBytesCount / 1024 / 1024, ReclaimedObjectsCount);
                if (reset)
                {
                    ReclaimedBytesCount = 0;
                    ReclaimedObjectsCount = 0;
            ...
            lock (this)
            {
                sb.Append(string.Format("CurMB: {0}| CurObj: {1}| ReclMB: {2}| ReclObj: {3}", _curBytes / 1024 / 1024,
                              CountObj(), ReclaimedBytesCount / 1024 / 1024, ReclaimedObjectsCount) +
                          Environment.NewLine);
                foreach (var item in _pool)
                {
                    ...
                    foreach (var a in item.Value)
                    {
                        ...
                        tmp = char.IsNumber(tmp[tmp.Length - 1])
                            ? new string(new[] {tmp[0], tmp[tmp.Length - 1]})
                            : tmp.Substring(0, 2);
                        sb.Append(tmp);
                        sb.Append(" ");
                    }
        ...
        {
            var ret = 0;
            foreach (var item in _pool) ret += item.Value.Count;
            return ret;
        }
        ...
        /// <remarks>
        ///     This function will return, if a garbage collector run and all subsequent finalizers has been completed.
        ///     <para>
        ///         After the function returns, all pending arrays not referenced from anywhere will have been placed into the
        ///         pool for reusement.
        ///     </para>
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        ///     This function will return, if a garbage collector run and all subsequent
        ///     finalizers has been completed.
        ///     <para>
        ///         After the function returns, all pending arrays not referenced from anywhere will have been placed
