    ...
    public class DecompositionLevel
    {
        ...
        public List<Disturbance> GetDisturbances(double threshold = 0.1, int minimunDistance = 3)
        {
            ...
            for (var i = 0; i < samples.Count; i++)
                ...
                {
                    disturbances.Add(new Disturbance(startIndex, samples[i - 1].Key, Details.Length,
                    ...
                    {
                        disturbances.Add(new Disturbance(samples[i].Key, samples[i].Key, Details.Length,
                            Signal.Samples.Length));
                    }
                }

            if (disturbances.Count > 1 &&
                disturbances[disturbances.Count - 1].Finish - disturbances[disturbances.Count - 2].Finish <
                minimunDistance)
            {
                disturbances[disturbances.Count - 2] = new Disturbance(disturbances[disturbances.Count - 2].Start,
                    disturbances[disturbances.Count - 1].Finish, Details.Length, Signal.Samples.Length);
        ...
        public class Disturbance
        {
            ...
            internal Disturbance(int start, int finish, int detailsLength, int signalLength)
            {
                ...
                if (start % 2 != 0 && finish % 2 != 0) SignalFinish--;
            }
        }
    }
}