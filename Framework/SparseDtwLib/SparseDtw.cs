...
namespace SparseDtwLib
{
    public class SparseDtw : ISignatureVerification
    {
        ...
        public VerificationResponse CheckSignature(List<SignatureSampleDeserialized> origSignature,
            List<List<RawPoint>> checkedSample, SignatureModel signatureModel = null)
        {
            ...
            for (var i = 0; i < features.Count; i++)
            {
                ...
                for (var j = 0; j < features.Count; j++)
                {
                    ...
                    tasks.Add(Task.Run(async () =>
                        await CompareSequences(features[closureI].Features.Select(f => f[featureName]),
                            features[closureJ].Features.Select(f => f[featureName]), 0.5)));
                }

                Task.WaitAll(tasks.ToArray());
                foreach (var task in tasks) distances.Add(task.Result);
        ...
        public List<NameMinMax> BuildModel(List<SignatureSampleDeserialized> origSignature)
        {
            ...
            Task.WaitAll(tasks.ToArray());
            foreach (var task in tasks) model.Add(task.Result);
            ...
            List<List<RawPoint>> checkedSample)
        {
            ...
            foreach (var featureName in CompareFeatureList)
            {
                ...
                foreach (var feature in features)
                    tasks.Add(Task.Run(async () => await CompareSequences(feature.Features.Select(f => f[featureName]),
                        checkedFeatures.Features.Select(f => f[featureName]), 0.5)));
        ...
        public async Task<double> CompareSequences(IEnumerable<double> sEnumerable, IEnumerable<double> qEnumerable,
            double res)
        {
